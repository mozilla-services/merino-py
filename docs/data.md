...
### Merino-fleece Search Term Sanitization

- `INFO web.suggest.sanitized` - A search term submitted to merino-fleece was
  classified as non-PII by the background sanitization pass. Only terms detected
  as `NON_PII` are logged; terms detected as any other PII type (email, numeric,
  person, etc.) are never written out. Terms without a query are skipped, so the
  record count is slightly below the `search_terms.sanitize` metric's
  `type="non_pii"` count, which includes them. Emission is gated by
  `[default.sanitize] log_search_terms` (`MERINO_FLEECE_SANITIZE__LOG_SEARCH_TERMS`),
  which is off by default. **Fields:**
  ...
  - `timestamp` - The UTC instant Merino submitted the search term to merino-fleece,
    ...
    offset, so the log sink's BigQuery dataset reads it as a `TIMESTAMP`. Named
    `submitted_at` on the wire between the two services.
  - `session_id` - A UUID generated by the client for each search session.
  - `sequence_no` - A client-side event counter (0-based) that records the query
    sequence within each search session.
  - `country` - The country the request came from.
...
- `recommendation.ml.contextual.last_updated` -
 ...
 updated in GCS and the current time.
- `recommendation.ml.cohort_model.update.timing` - A timer to measure the duration (in ms) of updating the cohort model data from GCS.
