...
- **Sanitization Pub/Sub Worker**, located in @apps/fleece/merino_fleece/sanitize/worker/, a separate process consuming the Pub/Sub backup channel **merino** falls back to when direct submission fails. Uses `gcloud-aio-pubsub`, whose subscriber is native asyncio, so the per-message handler awaits the shared sanitization pass directly on the loop that owns the SpaCy detector. `subscribe()` acks a message only after its handler returns and nacks it if the handler raises, so the handler returns on an unparseable message (acking a poison pill) and raises on failure or timeout. `main.py` stands up the same dependencies as the app lifespan and lets a dead subscriber propagate so the pod is recycled. Configured under `[default.pubsub]`.
