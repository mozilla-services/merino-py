...
def _build_validators() -> list[Validator]:
    ...
    return [
        ...
        Validator("mars.countries", is_type_of=list, must_exist=True),
        Validator("mars.form_factors", is_type_of=list, must_exist=True),
        Validator("mars.connect_timeout_sec", is_type_of=float, gt=0, must_exist=True),
        ...
        Validator("pubsub.ack_deadline_sec", is_type_of=float, gt=0, must_exist=True),
        Validator(
            "logging.format",
            is_in=["mozlog", "pretty"],
            must_exist=True,
