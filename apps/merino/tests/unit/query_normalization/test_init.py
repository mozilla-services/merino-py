...
async def test_init_pipeline_local() -> None:
    ...
    with patch("merino.utils.query_processing.normalization.settings") as mock_settings:
        ...
        mock_settings.query_normalization.sports_teams_file = (
            "apps/merino/tests/data/query_normalization/sports_teams.json"
        )
        mock_settings.query_normalization.word_freq_file = (
            "apps/merino/tests/data/query_normalization/word_freq.csv"
        )
        mock_settings.query_normalization.finance_tickers_file = (
            "apps/merino/tests/data/query_normalization/finance_tickers.json"
        )
        await init_pipeline()
