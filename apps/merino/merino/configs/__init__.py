...
_MERINO_PACKAGE_ROOT = Path(__file__).resolve().parent.parent
...
_validators = [
    ...
    Validator("query_pattern_matching.enabled", is_type_of=bool),
    Validator("query_pattern_matching.sample_rate", gte=0, lte=1),
    Validator("query_pattern_matching.patterns", is_type_of=list),
    Validator("fleece.url_base", is_type_of=str),
    Validator("fleece.pii_path", is_type_of=str),
    ...
    Validator("message_handler.max_queue_size", is_type_of=int, gt=0),
    ...
    Validator("providers.adm.backend", is_in=["mars", "test"]),
    ...
    Validator(
        ...
        is_in=[
            "dev/top_picks.json",
            "apps/merino/tests/data/top_picks.json",
...
settings = Dynaconf(
    root_path=_MERINO_PACKAGE_ROOT,
