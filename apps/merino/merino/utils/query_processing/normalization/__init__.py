...
async def init_pipeline() -> None:
    ...
    Should only be called once at startup. Errors are caught and logged
    so that a failure here does not prevent Merino from starting.
    """
    global _pipeline
    ...
    try:
        ...
        logger.exception(
            "Failed to initialize query normalization pipeline. "
            "Normalization will be disabled for this instance.",
            extra={"elapsed": timer() - start},
        )
        _pipeline = None
