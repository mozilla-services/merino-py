...
Also read the provider's registration in `apps/merino/merino/providers/suggest/manager.py` (find the matching `ProviderType` and `case` block).
...
- [ ] Uses `packages/merino-common/merino_common/utils/cron.py` `Job` class for periodic refresh
