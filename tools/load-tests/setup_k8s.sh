...
TARGET='https://merino.services.allizom.org'
...
MERINO_DIRECTORY=$DIRECTORY/tools/load-tests/kubernetes-config
...
SetupGksCluster() {
    ...
    $GCLOUD builds submit --config=./tools/load-tests/cloudbuild.yaml --substitutions=TAG_NAME="$LOCUST_IMAGE_TAG"
