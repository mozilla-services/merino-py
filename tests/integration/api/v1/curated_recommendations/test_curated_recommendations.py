...
from merino.curated_recommendations import CorpusApiBackend, CuratedRecommendationsProvider
from merino.curated_recommendations import corpus_api_provider
...
def fixture_mock_corpus_http_client() -> AsyncMock:
    """Mocks curated corpus api HTTP client."""
    # load json mock data
...
async def test_curated_recommendations_locale(client: TestClient):
    """Tests the curated recommendations endpoint response is as expected."""
    ...
    expected_recommendation = CuratedRecommendation(
        ...
        imageUrl=HttpUrl(
            "https://s3.us-east-1.amazonaws.com/pocket-curatedcorpusapi-prod-images/87fd6901-5bf5-4b12-8bde"
            "-24b86be79003.jpeg"
        ),
        receivedRank=1,
    )
...
async def test_curated_recommendations_locale_bad_request(client: TestClient):
    """Tests the curated recommendations endpoint response is 400 if locale is not provided"""
    # Mock the endpoint
