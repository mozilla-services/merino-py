...
async def test_fetch(sections_backend: SectionsBackend):
    ...
    assert len(crawl_sections) >= 10, f"Expected at least 10 _crawl sections, got {len(crawl_sections)}"
    ...
    assert len(non_crawl_sections) >= 10, f"Expected at least 10 non-_crawl sections, got {len(non_crawl_sections)}"
