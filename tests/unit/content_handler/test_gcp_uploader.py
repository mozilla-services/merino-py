...
def test_file_exists_in_gcs_returns_true(
    mock_gcs_client,
    mock_gcs_bucket,
    mock_gcs_blob,
    test_project_name,
    ...
    """Test that file_exists_in_gcs returns True when blob exists."""
    mock_gcs_client.bucket.return_value = mock_gcs_bucket
    mock_gcs_bucket.blob.return_value = mock_gcs_blob
    mock_gcs_blob.exists.return_value = True

    gcs_uploader = GcsUploader(test_project_name, test_bucket_name, test_https_cdn_host_name)
    result = gcs_uploader.file_exists_in_gcs(test_destination_name)

    assert result is True
    mock_gcs_blob.exists.assert_called_once()


def test_file_exists_in_gcs_returns_false(
    mock_gcs_client,
    mock_gcs_bucket,
    mock_gcs_blob,
    test_project_name,
    ...
    """Test that file_exists_in_gcs returns False when blob does not exist."""
    mock_gcs_client.bucket.return_value = mock_gcs_bucket
    mock_gcs_bucket.blob.return_value = mock_gcs_blob
    mock_gcs_blob.exists.return_value = False
