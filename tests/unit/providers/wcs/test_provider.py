...
from merino.providers.wcs.protocol import TeamsResponse, LiveMatchesResponse
...
async def test_live_matches_sorted_ascending_by_date() -> None:
    ...
    matches: LiveMatchesResponse = await WcsProvider(
        settings=settings.providers.sports
    ).get_live_matches(team_keys=None)
...
async def test_get_teams_count() -> None:
    ...
    response = WcsProvider(settings=settings.providers.sports).get_teams()
