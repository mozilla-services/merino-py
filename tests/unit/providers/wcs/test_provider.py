...
def test_limit_keeps_closest_to_target_date() -> None:
    ...
    full = WcsProvider(settings=settings.providers.sports).get_matches(
        _ANCHOR, limit=None, team_keys=None
    )
    limited = WcsProvider(settings=settings.providers.sports).get_matches(
        _ANCHOR, limit=1, team_keys=None
    )
...
def test_response_is_deterministic_for_same_anchor() -> None:
    ...
    a = WcsProvider(settings=settings.providers.sports).get_matches(
        _ANCHOR, limit=None, team_keys=None
    )
    b = WcsProvider(settings=settings.providers.sports).get_matches(
        _ANCHOR, limit=None, team_keys=None
    )
...
def test_live_is_deterministic_within_same_utc_day() -> None:
    ...
    a = WcsProvider(settings=settings.providers.sports).get_live_matches(team_keys=None)
    b = WcsProvider(settings=settings.providers.sports).get_live_matches(team_keys=None)
    assert a.model_dump() == b.model_dump()
