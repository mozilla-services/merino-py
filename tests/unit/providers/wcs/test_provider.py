...
def _provider() -> WcsProvider:
    return WcsProvider(backend=DefaultWcsBackend())
...
def test_buckets_split_by_date() -> None:
    ...
    response = _provider().get_matches(_ANCHOR, limit=None, team_keys=None)
...
def test_limit_keeps_closest_to_target_date() -> None:
    ...
    full = _provider().get_matches(_ANCHOR, limit=None, team_keys=None)
    limited = _provider().get_matches(_ANCHOR, limit=1, team_keys=None)
...
def test_teams_filter_matches_either_side() -> None:
    ...
    response = _provider().get_matches(_ANCHOR, limit=None, team_keys=frozenset({"BRA"}))
...
def test_response_is_deterministic_for_same_anchor() -> None:
    ...
    a = _provider().get_matches(_ANCHOR, limit=None, team_keys=None)
    b = _provider().get_matches(_ANCHOR, limit=None, team_keys=None)
...
def test_live_returns_only_in_progress_events() -> None:
    ...
    response = _provider().get_live_matches(team_keys=None)
...
def test_live_matches_sorted_ascending_by_date() -> None:
    ...
    matches = _provider().get_live_matches(team_keys=None).matches
...
def test_live_teams_filter_matches_either_side() -> None:
    ...
    response = _provider().get_live_matches(team_keys=frozenset({"BRA"}))
...
def test_live_unknown_team_returns_empty() -> None:
    ...
    response = _provider().get_live_matches(team_keys=frozenset({"ZZZ"}))
    assert response.matches == []


def test_live_is_deterministic_within_same_utc_day() -> None:
    """Two calls in the same UTC day produce identical payloads."""
    a = _provider().get_live_matches(team_keys=None)
    b = _provider().get_live_matches(team_keys=None)
    assert a.model_dump() == b.model_dump()


def test_event_info_contract() -> None:
    """Every EventInfo exposes required typed fields."""
    response = _provider().get_matches(_ANCHOR, limit=1, team_keys=None)
    events = response.previous + response.current + response.next_
