...
async def test_initialize_skip(
    ...
    mocker.patch(
        "merino.providers.suggest.top_picks.backends.top_picks.TopPicksBackend.fetch"
...
async def test_fetch_top_picks_data_fails(
    ...
    records = filter_caplog(caplog.records, "merino.providers.suggest.top_picks.provider")
...
async def test_query(
    ...
    result: list[BaseSuggestion] = await top_picks.query(srequest(query, None, None, None))
    assert result == [
        Suggestion(
            block_id=0,
            title=title,
...
async def test_query_filtered_input(
    ...
    assert await top_picks.query(srequest(query, None, None, None)) == []
...
async def test_short_domain_query(
    ...
    result = await top_picks.query(srequest(query, None, None, None))
