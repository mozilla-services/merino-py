...
@pytest.fixture(name="accuweather_city_parameters")
def fixture_accuweather_city_parameters(
...
def test_init_api_key_value_error(
    mocker: MockerFixture, accuweather_city_parameters: dict[str, Any]
) -> None:
    ...
    accuweather_city_parameters["api_key"] = ""
...
def test_init_url_value_error(
    mocker: MockerFixture, accuweather_city_parameters: dict[str, Any], url_value: str
) -> None:
    ...
    accuweather_city_parameters[url_value] = ""

    with pytest.raises(ValueError) as accuweather_error:
        AccuweatherCityBackend(
            cache=mocker.AsyncMock(spec=RedisAdapter), **accuweather_city_parameters
        )
...
async def test_get_weather_report(
    ...
    mocker.patch(
        "merino.providers.weather.backends.accuweather.AccuweatherCityBackend"
...
async def test_get_weather_report_from_cache(
    ...
    accuweather: AccuweatherCityBackend = AccuweatherCityBackend(
        cache=RedisAdapter(redis_mock), **accuweather_city_parameters
    )
    client_mock: AsyncMock = cast(AsyncMock, accuweather.http_client)
...
async def test_get_weather_report_failed_location_query(
    accuweather: AccuweatherCityBackend,
    geolocation: Location,
    response_header: dict[str, str],
) -> None:
...
async def test_get_weather_report_invalid_location(
    accuweather: AccuweatherCityBackend, location: Location
) -> None:
...
async def test_get_location_no_location_returned(
    accuweather: AccuweatherCityBackend, response_header: dict[str, str]
) -> None:
...
@pytest.mark.asyncio
async def test_get_location_error(accuweather: AccuweatherCityBackend) -> None:
    ...
    with pytest.raises(AccuweatherError) as accuweather_error:
        await accuweather.get_location(country, region, city)
...
async def test_get_current_conditions(
    ...
    accuweather: AccuweatherCityBackend = request.getfixturevalue(accuweather_fixture)
    client_mock: AsyncMock = cast(AsyncMock, accuweather.http_client)
    client_mock.get.return_value = Response(
        status_code=200,
        headers=response_header,
...
@pytest.mark.asyncio
async def test_get_forecast_error(accuweather: AccuweatherCityBackend) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            f"AccuweatherCityBackend:v4:localhost:"
            f"{hashlib.blake2s('q'.encode('utf-8') + 'asdfg'.encode('utf-8')).hexdigest()}",
        ),
        (
            {},
            "AccuweatherCityBackend:v4:localhost",
        ),
        (
            {"q": "asdfg"},
            f"AccuweatherCityBackend:v4:localhost:"
...
async def test_get_request_cache_store_errors(
    ...
    accuweather: AccuweatherCityBackend = AccuweatherCityBackend(
        cache=redis_mock, **accuweather_city_parameters
    )
