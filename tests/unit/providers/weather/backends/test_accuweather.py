...
@pytest.mark.parametrize(
    ...
    [
        {("US", "SanFrancisco"), "CA"},
        {("US", "SanFrancisco"), None},
    ],
    ids=["Region", "Fallback"],
)
@pytest.mark.asyncio
async def test_get_weather_report_with_successful_regions_mapping_match(
    mocker: MockerFixture,
    accuweather: AccuweatherBackend,
    geolocation: Location,
    accuweather_location_response: bytes,
    ...
    regions_mapping: dict[tuple[str, str], str | None],
) -> None:
    """Test that the get_weather_report method returns when weather with successful regions match."""
    caplog.set_level(logging.WARN)

    mocker.patch(
        "merino.providers.weather.backends.accuweather.pathfinder" ".SUCCESSFUL_REGIONS_MAPPING"
    ).return_value = regions_mapping
