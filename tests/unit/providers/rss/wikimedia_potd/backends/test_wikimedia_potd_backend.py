...
class TestGcsBucketPaths:
    """Tests that assets are written to and read from the expected dated bucket paths."""

    @pytest.mark.parametrize(
        ("is_thumbnail", "expected_path"),
        [
            (True, "rss/wikimedia_potd/2026-06-07/thumbnail.jpeg"),
            (False, "rss/wikimedia_potd/2026-06-07/hi_res.jpeg"),
        ],
        ids=["thumbnail", "hi_res"],
    )
    @freezegun.freeze_time("2026-06-07")
    def test_upload_potd_image_uses_dated_bucket_path(
        self,
        backend: WikimediaPictureOfTheDayBackend,
        is_thumbnail: bool,
        expected_path: str,
    ) -> None:
        """Uploads the image to the dated bucket directory with the variant file name."""
        backend.gcs_uploader.upload_image.return_value = (  # type: ignore[attr-defined]
            "https://cdn/rss/wikimedia_potd/image.jpeg"
        )
        backend.gcs_uploader.get_file_by_name.return_value = MagicMock()  # type: ignore[attr-defined]

        backend.upload_potd_image(
            image=Image(content=b"255", content_type="image/jpeg"), is_thumbnail=is_thumbnail
        )

        _, kwargs = backend.gcs_uploader.upload_image.call_args  # type: ignore[attr-defined]
        assert kwargs["destination_name"] == expected_path

    @freezegun.freeze_time("2026-06-07")
    def test_manifest_upload_and_fetch_use_same_dated_path(
        self, backend: WikimediaPictureOfTheDayBackend, potd: PictureOfTheDay
    ) -> None:
        """The manifest is fetched from the exact path it was uploaded to."""
        expected_path = "rss/wikimedia_potd/2026-06-07/manifest.json"
        # a truthy blob so the upload verification does not raise; None on fetch so the
        # method returns without needing a valid manifest payload to validate.
        backend.gcs_uploader.get_file_by_name.return_value = MagicMock()  # type: ignore[attr-defined]

        backend.upload_potd_manifest(potd)

        _, upload_kwargs = backend.gcs_uploader.upload_content.call_args  # type: ignore[attr-defined]
        assert upload_kwargs["destination_name"] == expected_path

        backend.gcs_uploader.get_file_by_name.reset_mock()  # type: ignore[attr-defined]
        backend.gcs_uploader.get_file_by_name.return_value = None  # type: ignore[attr-defined]

        backend.fetch_potd_from_gcs_bucket()

        fetch_args, _ = backend.gcs_uploader.get_file_by_name.call_args  # type: ignore[attr-defined]
        assert fetch_args[0] == expected_path
