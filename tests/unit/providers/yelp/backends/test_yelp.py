...
async def test_get_businesses_success(
    ...
    actual = await yelp.get_businesses(term, location)
    assert actual == expected
    assert actual == expected


@pytest.mark.asyncio
async def test_get_businesses_bad_response(
    yelp: YelpBackend,
    ...
    _ = await yelp.get_businesses(term, location)
    records = filter_caplog(caplog.records, "merino.providers.suggest.yelp.backends.yelp")

    assert len(caplog.records) == 1

    assert records[0].message.startswith("Yelp business response json has incorrect shape")


@pytest.mark.asyncio
async def test_get_businesses_failure_for_http_500(
    yelp: YelpBackend,
    caplog: LogCaptureFixture,
    filter_caplog: FilterCaplogFixture,
) -> None:
    ...
    assert len(caplog.records) == 1
    assert records[0].message.startswith("Yelp request error")
