...
class TestProcessLinkFavicons:
    ...
    def test_process_link_favicons_sort_prevents_cutoff(self, favicon_extractor):
        ...
        links = [
            {"href": f"/icon{i}.png", "rel": ["icon"]} for i in range(4)
        ] + [{"href": "/apple-touch.png", "rel": ["apple-touch-icon"]}]
...
class TestProcessAppleTouchIconFallback:
    ...
    @pytest.mark.asyncio
    async def test_apple_touch_icon_fallback_success(self, favicon_extractor, mock_favicon_scraper):
        ...
        result = await favicon_extractor._process_apple_touch_icon_fallback(
            "https://example.com"
        )
