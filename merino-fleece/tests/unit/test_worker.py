"""Unit tests for merino_fleece.worker.worker."""
...
from merino_fleece.worker.worker import FleeceQueueWorker, callback
...
def test_callback_sanitizes_emits_and_acks(mocker: MockerFixture) -> None:
    ...
    emit = mocker.patch("merino_fleece.worker.worker.emit_sanitized_query")
...
def test_callback_drops_invalid_message(
    ...
    with caplog.at_level("ERROR", logger="merino_fleece.worker.worker"):
        callback(message)
...
def subscriber_mock(mocker: MockerFixture) -> MagicMock:
    ...
    client_cls = mocker.patch("merino_fleece.worker.worker.pubsub_v1.SubscriberClient")
...
def test_stop_prevents_reconnect_after_error(
    ...
    sleep = mocker.patch("merino_fleece.worker.worker.time.sleep")
    worker = FleeceQueueWorker("my-subscription")
    future = subscriber_mock.subscribe.return_value
...
def test_restart_ignored_while_subscriber_open(
    ...
    with caplog.at_level("WARNING", logger="merino_fleece.worker.worker"):
        worker.restart()
