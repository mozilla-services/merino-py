"""Static hex colour data for WCS national teams, keyed by 3-letter team code."""

TEAM_COLOURS: dict[str, list[str]] = {
    "ALG": ["#006233", "#FFFFFF", "#D21034"],
    "ARG": ["#74ACDF", "#FFFFFF", "#F6B40E"],
    "AUS": ["#012169", "#FFFFFF", "#E4002B"],
    "AUT": ["#ED2939", "#FFFFFF"],
    "BEL": ["#000000", "#FFD90C", "#EF3340"],
    "BIH": ["#002395", "#FECB00", "#FFFFFF"],
    "BRA": ["#009C3B", "#FFDF00", "#002776"],
    "CAN": ["#FF0000", "#FFFFFF", "#D52B1E"],
    "CDR": ["#00A3E0", "#FFD100", "#EF3340"],
    "CHE": ["#FF0000", "#FFFFFF"],
    "CIV": ["#F77F00", "#FFFFFF", "#009E60"],
    "COL": ["#FCD116", "#003893", "#CE1126"],
    "CUW": ["#002B7F", "#F9E814", "#FFFFFF"],
    "CVI": ["#003893", "#FFFFFF", "#CF2027"],
    "CZE": ["#11457E", "#FFFFFF", "#D7141A"],
    "ECU": ["#FCD116", "#003893", "#CE1126"],
    "EGY": ["#CE1126", "#FFFFFF", "#000000"],
    "ENG": ["#FFFFFF", "#CE1126"],
    "ESP": ["#AA151B", "#F1BF00"],
    "FRA": ["#0055A4", "#FFFFFF", "#EF4135"],
    "GER": ["#000000", "#DD0000", "#FFCE00"],
    "GHA": ["#CE1126", "#FCD116", "#006B3F"],
    "HAI": ["#00209F", "#D21034", "#FFFFFF"],
    "HRV": ["#FF0000", "#FFFFFF", "#171796"],
    "IRN": ["#239F40", "#FFFFFF", "#DA0000"],
    "IRQ": ["#CE1126", "#FFFFFF", "#000000"],
    "JOR": ["#000000", "#FFFFFF", "#007A3D"],
    "JPN": ["#FFFFFF", "#BC002D"],
    "KOR": ["#FFFFFF", "#CD2E3A", "#0047A0"],
    "KSA": ["#006C35", "#FFFFFF"],
    "MAR": ["#C1272D", "#006233"],
    "MEX": ["#006847", "#FFFFFF", "#CE1126"],
    "NLD": ["#AE1C28", "#FFFFFF", "#21468B"],
    "NOR": ["#BA0C2F", "#FFFFFF", "#00205B"],
    "NZL": ["#00247D", "#FFFFFF", "#CC142B"],
    "PAN": ["#FFFFFF", "#005293", "#D21034"],
    "PAR": ["#D52B1E", "#FFFFFF", "#0038A8"],
    "PRT": ["#006600", "#FF0000", "#FFCC00"],
    "QAT": ["#8A1538", "#FFFFFF"],
    "RSA": ["#007749", "#FFB81C", "#DE3831"],
    "SCO": ["#0065BD", "#FFFFFF"],
    "SEN": ["#00853F", "#FDEF42", "#E31B23"],
    "SWE": ["#006AA7", "#FECC00"],
    "TUN": ["#E70013", "#FFFFFF"],
    "TUR": ["#E30A17", "#FFFFFF"],
    "URY": ["#FFFFFF", "#0038A8", "#FCD116"],
    "USA": ["#B22234", "#FFFFFF", "#3C3B6E"],
    "UZB": ["#1EB53A", "#0099B5", "#FFFFFF"],
}
