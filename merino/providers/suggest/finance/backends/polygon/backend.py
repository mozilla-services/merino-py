...
class PolygonBackend(FinanceBackend):
    ...
    async def bulk_download_and_upload_ticker_images(
        ...
        bucket = self.gcs_uploader.storage_client.bucket(self.gcs_uploader.bucket_name)
        ...
        for ticker in tickers:
            try:
                ...
                blob: Blob = bucket.blob(destination_name)
                if self.gcs_uploader.file_exists_in_gcs(blob):
                    public_url = self.gcs_uploader._get_public_url(blob, destination_name)
                    logger.info(
                        f"Skipping upload for {ticker}: blob already exists at {public_url}"
                    )
