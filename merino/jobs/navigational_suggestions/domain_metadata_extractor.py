...
class DomainMetadataExtractor:
    ...
    def _get_default_favicon(self, url: str) -> Optional[str]:
        ...
        response = requests.get(url, headers={'User-agent': self.FIREFOX_UA}, timeout=self.TIMEOUT)
            ...
            if width is None:
                try:
                    ...
                    with Image.open(BytesIO(response.content)) as img:
                        ...
                        if width != height:
                            logger.info(f'favicon shape "{width}*{height}" is not square')
                            width = min(width, height)
                except Exception as e:
                    logger.info(f'Exception "{str(e)}" while getting best favicon {url}')
                    pass
