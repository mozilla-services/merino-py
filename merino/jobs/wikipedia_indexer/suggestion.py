...
class Scorer:
    ...
    def score(self, doc: Dict) -> float:
        ...
        incoming_links = self._score_norm_log2(
            doc.get("incoming_links", 0), self.incoming_links_norm
        )
        page_size = self._score_norm_log2(doc.get("text_bytes", 0), self.PAGE_SIZE_NORM)
        external_links = self._score_norm(
            len(doc.get("external_links", [])), self.EXTERNAL_LINKS_NORM
        )
        heading = self._score_norm(len(doc.get("heading", [])), self.HEADING_NORM)
        redirect = self._score_norm(len(doc.get("redirect", [])), self.REDIRECT_NORM)
        ...
        score = (
            score
            / (
                self.INCOMING_LINKS_WEIGHT
                + self.EXTERNAL_LINKS_WEIGHT
                + self.PAGE_SIZE_WEIGHT
                + self.HEADING_WEIGHT
...
class Builder:
    """Accepts a document and builds scored suggestions."""
